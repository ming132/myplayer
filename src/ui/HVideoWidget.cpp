...
void HVideoWidget::start() {
    ...
    if (!pImpl_player) {
        ...
        updateUI();
        Aplayer= std::thread([this] {APlayer();});
        Vplayer= std::thread([this] {VPlayer();});
    }
    else {
        if (status == PAUSE) {
            resume();
...
void HVideoWidget::resume() {
    if (status == PAUSE && pImpl_player) {
        pImpl_player->resume();
        //timer->start(1000 / hyper_para/(fps ? fps : pImpl_player->fps));
        status = PLAY;
...
void HVideoWidget::APlayer()
{
    ...
    while(status!=STOP)
    {
        while(status ==PAUSE)
            std::this_thread::yield();
        if (pImpl_player->pop_frame(&audiownd->last_frame) == 0)
        {
            ats=audiownd->last_frame.ts;
            audiownd->update();
        }
    }
}
...
//    if (pImpl_player == NULL)   return;
//    //if (audiownd->get_remainder()>=8) return ;
//    if (pImpl_player->pop_frame(&videownd->last_frame) == 0) {
//        // update progress bar
//        if (toolbar->sldProgress->isVisible()) {
//            int progress = (videownd->last_frame.ts - pImpl_player->start_time) / 1000;
...
////    {
////         audiownd->update();
////         //std::cout<<audiownd->last_frame.ts<<" !!\n";
////         ats=audiownd->last_frame.ts;
////    }
//}
